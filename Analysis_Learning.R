...
write.csv(R,file="write the group and orientation of the stimuli (e.g. PI_Training_3Sq_Bottom).csv", sep = ",", row.names = TRUE, col.names = TRUE)
...
library(ggplus)
pdf("individual_trajectories.pdf")
...
group_names = levels(R$Group)


pdf("Transition plots by Group.pdf")
...
pdf("Transition plots by Group and Sex.pdf")
...
#Boxplot
PI_Plot<-ggplot  (Difference,aes(x=Group , y=PI)) +
        scale_x_discrete() +
        geom_jitter(aes(colour = ID), 
        ...
        theme(legend.position="none")
PI_Plot
ggsave(PI_Plot, filename='PI_Comparison.pdf')
